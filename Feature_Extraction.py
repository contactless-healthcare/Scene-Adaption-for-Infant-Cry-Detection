###########################################################################
#   The feature extraction are baed on the pyAudioAnalysis
...
def main():
    # Load segmentation
    dataDir = f"Data\\Crying_Github.dat"
    audioData = Data_Acq(dataDir)

    # feature extraction
    acoustic_feature, spectrogram = acoustic_features_and_spectrogram(audioData["data"])
    audioData["acoustic_feature"] = acoustic_feature
    audioData["spectrogram"] = spectrogram

    # save
    file = open(dataDir, 'wb')
    pickle.dump(audioData, file)
    file.close()

    # saveDir = f"Data\\Crying_Github.dat"
    # sample = Data_Acq(saveDir)
    # print(sample)
